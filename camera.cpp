...
void Camera::Reset() { camera_up = glm::vec3(0, 1, 0); }
...
void Camera::Update() {
  ...
  if (camera_type == CameraType::ORTHO) {
    ...
    projection = glm::ortho(-1.5f * float(aspect), 1.5f * float(aspect), -1.5f,
                            1.5f, -10.0f, 10.f);
  ...
  gluLookAt(camera_position.x, camera_position.y, camera_position.z,
            camera_look_at.x, camera_look_at.y, camera_look_at.z, camera_up.x,
            camera_up.y, camera_up.z);
}
...
void Camera::SetCameraFree() { cameraMode = CameraModes::WORLD; }
...
void Camera::SetLookAt(glm::vec3 pos) { camera_look_at = pos; }
void Camera::SetFOV(double fov) { field_of_view = fov; }
  ...
  if (camera_type == CameraType::FREE) {
    ...
    case LEFT:
      camera_position_delta -=
          glm::cross(camera_direction, camera_up) * camera_scale;
      break;
    case RIGHT:
      camera_position_delta +=
          glm::cross(camera_direction, camera_up) * camera_scale;
...
void Camera::ChangeHeading(float degrees) {
  ...
  // up or down The heading delta direction changes
...
CameraType::Enum Camera::GetMode() { return camera_type; }
  ...
  if (readTimeline) {
    ...
    if (!isExecuting) {
      ...
      if (!strcmp(command, "FOLLOW")) {
        char whichQuad[10];
        glm::vec3 position;
        sscanf(delayedCommand, "%lf %lf %s %s %f %f %f", &nextTime, &comTime,
               command, whichQuad, &position.x, &position.y, &position.z);
        ...
        */
        SetCameraModeFollowUpright(Quadrotor::getQuadFromName(whichQuad),
                                   -position);
        ...
        SetCameraModeFollowFront(Quadrotor::getQuadFromName(whichQuad),
                                 -position);
        ...
        glm::vec3 center;
        sscanf(delayedCommand, "%lf %lf %s %f %f %f %lf", &nextTime, &comTime,
               command, &center.x, &center.y, &center.z, &comAngle);
        SetCameraModeCircleMotion(center, camera_position);

      } else if (!strcmp(command, "LOOKATPOINT")) {
        glm::vec3 center;
        sscanf(delayedCommand, "%lf %lf %s %f %f %f", &nextTime, &comTime,
               command, &center.x, &center.y, &center.z);
        SetLookAt(center);
        ...
        glm::vec3 center;
        char whichQuad[10];
        sscanf(delayedCommand, "%lf %lf %s %s %f %f", &nextTime, &comTime,
               command, whichQuad);
        ...
        sscanf(delayedCommand, "%lf %lf %s %lf %f %f %f", &nextTime, &comTime,
               command, &comOrientationTime, &comVect.x, &comVect.y,
               &comVect.z);
        isExecuting = true;
        ...
        comDirection =
            comVect -
            camera_position; // glm::vec3(comVect.x-camera_position.x,comVect.y-camera_position.y,comVect.z-camera_position.z);
                             //  comDirection = glm::normalize(comDirection);
      }
      // isExecuting = false;
      isMoving = true;
    } else if (current_time <= comOrientationTime) // dont have to worry about
                                                   // modes other than MOV
    {
      if (cameraMode == CameraModes::LOOKATPOINT ||
          cameraMode == CameraModes::LOOKATQUAD)
        return;
      ...
      float angle_axis =
          glm::angle(cam_direction, glm::normalize(comDirection));
      //  printf("Angle: %f\n",angle_axis);

      angle_axis = angle_axis * (float)(timediff / delta_time);
      if (glm::angle(glm::normalize(comRotationAxis), glm::vec3(0, 1, 0)) >
          glm::half_pi<float>()) {
        camera_up = -comRotationAxis;
      ...
      if (!strcmp(command, "MOV")) {
        ...
        glm::vec3 dist2 =
            distance * (float)(cameraTime.getTimeDiffSec() / delta_time);
...
/*
void Camera::loadQuadrotors(NeoQuad* neoQuad, Dronedemort* dronedemort,
MamaQuad* mamaQuad)
{
...
*/
glm::vec3 Camera::GetCameraPosition() { return camera_position; }

glm::vec3 Camera::GetLookAt() { return camera_look_at; }
